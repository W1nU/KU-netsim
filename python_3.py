import os, shutil, sys, io, glob, time, torch
from ns import ns
...
address2.SetBase(ns.network.Ipv4Address("10.2.2.0"), ns.network.Ipv4Mask("255.255.255.0"))
...
class Generator(ns.applications.Application):
    ...
    def __init__(self, node: ns.Node, send_address: ns.Ipv4Address):
        ...
        self.send_socket = ns.network.Socket.CreateSocket(node, ns.core.TypeId.LookupByName("ns3::UdpSocketFactory"))
    ...
    def StartApplication(self):
        self.send_socket.Connect(ns.network.InetSocketAddress(self.send_address, self.port).ConvertTo())
        self.ScheduleTransmit(ns.core.Seconds(0.))
    ...
    def ScheduleTransmit(self, delay): 
        ...
        for index in range(3):
            event = ns.GeneratorEventSend(Generator._Send, self.send_socket, address, index)
    ...
    def Send(self, address: ns.InetSocketAddress, index) -> None: 
        ...
        self.m_sent += 1
        print("At time +{s}s Generator sent {i}th packet of {b} bytes from {ip} port {port}"
...
class Forwarder(ns.applications.Application):
    ...
    def __init__(self, node: ns.Node, rx_address: ns.Ipv4Address, tx_address: ns.Ipv4Address):
        super().__init__()
        self.__python_owns__ = False # let C++ destroy this on Simulator::Destroy
        ...
        for lv, net in enumerate(self.nets):
			...
			'myScratch/model/Lv{}.pth'.format(lv)))
            net.eval()
    ...
    def Send(self, packet: ns.Packet) -> None:
		...
		#   -> reshaped to (3, H, W)
        if 3:
            x = torch.tensor(np.frombuffer(original_data,
							dtype=np.uint8)[128:],
							dtype=torch.float32).reshape(3, 32, 32) / 255
            for lv in range(3):
        ...
        # Packetize the result and forward to the next node
		####### removed #########

        print("At time +{s}s Forwarder sent a packet of {b} bytes from {ip} port {port}"
                .format(s=ns.Simulator.Now().GetSeconds(), 
    ...
    def Receive(self):
		...
		# Handling packets from Node 0
		####### removed #########
        #if pkt is from Node 0 
            print("At time +{s}s Forwarder received {b} bytes from {ip} port {port}"
                    .format(s=ns.Simulator.Now().GetSeconds(),
        ...
        event = ns.ForwarderEventSend(Forwarder._Send,
									self.send_socket,
									packet)
...
class Receiver(ns.applications.Application):
    ...
    def __init__(self, node: ns.Node, address: ns.Ipv4Address):
        ...
        self.receive_socket = ns.network.Socket.CreateSocket(node, ns.core.TypeId.LookupByName("ns3::UdpSocketFactory"))
        self.receive_socket.Bind(ns.network.InetSocketAddress(ns.network.Ipv4Address.GetAny(), self.port).ConvertTo())
    ...
    def Receive(self):
        ...
        if 3:
            x = torch.tensor(np.frombuffer(image_bytes,
                        dtype=np.float32)[32:],
                        dtype=torch.float32).reshape(self.lv_shapes[3])
            # Decode latent
            ...
            img = Image.fromarray(np.frombuffer(image_bytes,
                        dtype=np.uint8)[128:].reshape(self.lv_shapes[3]).transpose(1,2,0))

        # Store the image in the directory
        image_filename = os.path.join(self.imsave_dir, 'Arrival_{}.png'.format(self.m_received))
        img.save(image_filename)
            
        print("At time +{s}s Receiver received {b} bytes from {ip} port {port}"
              .format(s=ns.Simulator.Now().GetSeconds(),
                      b=packet.__deref__().GetSize(),
              ...
              file=sys.stderr,
              flush=True)
        print("At time +{}s Receiver stored the image {}"
              .format(ns.Simulator.Now().GetSeconds(),
                      self.m_received),
              file=sys.stderr,
              flush=True)
...
forwarderApp = Forwarder(nodes.Get(1),
				interface1.GetAddress(0), interface2.GetAddress(1))
